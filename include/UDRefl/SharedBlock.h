...
namespace Ubpa::UDRefl {
	...
	public:
		...
		SharedBlock(U* ptr, Deleter d, Alloc alloc) : ptr{ ptr, std::move(d), std::move(alloc) } {}
		template<typename Deleter, typename Alloc>
		SharedBlock(std::nullptr_t ptr, Deleter d, Alloc alloc) : ptr{ ptr, std::move(d), std::move(alloc) } {}
        ...
        template<typename T>
        const T* as_ptr() const noexcept { return const_cast<SharedBlock*>(this)->as_ptr<T>(); }
